...
const Slide = ({ slide, index, current, handleSlideClick }: SlideProps) => {
    ...
    const { poster_path, title, id } = slide;

    return (
        <div className="[perspective:1200px] [transform-style:preserve-3d]">
            <li
                ref={slideRef}
                className="flex flex-1 flex-col items-center justify-center relative text-center text-white opacity-100 transition-all duration-300 ease-in-out w-screen h-screen mx-[4vmin] z-10 "
            ...
            >
                <div
                    className="absolute top-0 left-0 w-full h-full bg-gray-950 rounded-[1%] overflow-hidden transition-all duration-150 ease-out"
                    style={{
                        transform:
                            current === index
                ...
                >
                    <h2 className="text-lg md:text-3xl lg:text-5xl font-serif font-extrabold">
                        {title}
                    </h2>
                    <div className="flex justify-center">
                        <Link to={`/movie-detail/${id}`} state={{ movieData: slide }}>
                            <button className="mt-6 px-6 py-2 w-fit mx-auto sm:text-lg text-black bg-white h-12 border cursor-pointer border-transparent text-xs flex justify-center items-center rounded-2xl hover:shadow-lg transition duration-200 shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)]">
                                Watch Now
                            </button>
                        </Link>
                    </div>
...
export function Carousel({ slides }: CarouselProps) {
    ...
    return (
        <div
            className="relative w-screen h-[70vmin] mx-autos"
